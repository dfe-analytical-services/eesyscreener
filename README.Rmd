...
- A core `screen_csv()` function that will screen a pair of data and meta CSV files. This is built from `screen_dfs()`, and `screen_filenames()` and the constituent individual `precheck_*()` and `check_*()` functions.
...
The minimal example above shows the output structure for a file that runs the whole pipeline. Below are code excerpts for creating three other common scenarios — a file that fails a check, a file that cannot be read at all, and a file that passes screening but is not suitable for publishing through the API.
