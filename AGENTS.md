...
Run R commands via `Rscript -e "..."` in the terminal. Chain multiple lines / expressions with `;`, e.g. `Rsript -e "devtools::load_all(); devtools::test()`.
...
Checks are organized into **stages**, executed sequentially. If any check fails in a stage, screening stops and returns the failure stage. Examples are:
