...
2. Determine the correct new name following the `check_<area>_<what>()` or `precheck_<area>_<what>()` naming convention
3. Create `R/<new_name>.R` with the migrated function and full roxygen2 documentation
4. Create `tests/testthat/test-<new_name>.R` with passing and failing test cases
5. Add the function call to the appropriate stage in `R/screen_dfs.R`
6. Update `_pkgdown.yml` to include the new function under the correct grouping
7. Run `devtools::document()`, then `devtools::test()` — fix any issues before proceeding
8. Simplify and improve the function following the guidelines and patterns in this project
9. Rerun `devtools::document()`, then `devtools::test()` — fix any issues before proceeding
10. Once everything passes, create a branch, commit the changes, and open a PR for review
11. If any GitHub Action fails on the PR, review the logs then fix, commit and push
