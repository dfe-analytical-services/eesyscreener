test_that("passes when geographic levels are acceptable values", {
  ...
  expect_equal(precheck_geog_level(data)$result, "PASS")
  expect_no_error(precheck_geog_level(
    data,
    stop_on_error = TRUE
  ))
  expect_no_error(precheck_geog_level(
...
test_that("fails when geographic levels are blanks or NAs", {
  ...
  expect_equal(precheck_geog_level(data)$result, "FAIL")
  expect_error(precheck_geog_level(
    data,
    stop_on_error = TRUE
  ))
})
