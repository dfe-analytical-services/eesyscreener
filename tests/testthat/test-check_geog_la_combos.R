...
test_that("fails with one invalid LA combination (singular)", {
  bad_data <- la_data |>
    dplyr::mutate(old_la_code = "999", new_la_code = "BADCODE", la_name = "BadLA")
...
test_that("fails with multiple invalid LA combinations (plural)", {
  bad_data <- rbind(
    la_data |>
      dplyr::mutate(old_la_code = "998", new_la_code = "BADCODE1", la_name = "BadLA1"),
    la_data |>
      dplyr::mutate(old_la_code = "999", new_la_code = "BADCODE2", la_name = "BadLA2")
  )
  result <- check_geog_la_combos(bad_data)
  expect_equal(result$result, "FAIL")
...
test_that("ignores rows where any LA column is NA", {
  na_data <- la_data |>
    dplyr::mutate(old_la_code = NA_character_, new_la_code = "BADCODE", la_name = "BadLA")
...
test_that("accepts z code combinations from universal geog options", {
  z_data <- la_data |>
    dplyr::mutate(old_la_code = "z", new_la_code = "z", la_name = "Not applicable")
