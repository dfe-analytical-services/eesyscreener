...
test_that("correctly identifies number of columns with spaces",{
  expect_identical(
    ...
    stop_on_error = FALSE)$message, "There are one or more spaces in 3 col_name values in the metadata file at columns: 1,3,5."
  )
})
...
test_that("correctly identifies which columns have spaces",{
  expect_identical(
    precheck_col_name_spaces(
      ...
      stop_on_error = FALSE)$message, "There are one or more spaces in 3 col_name values in the metadata file at columns: 1,4,5."
  )
})
