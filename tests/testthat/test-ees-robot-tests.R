...
# Checking that the screener runs to completion on all robot tests
test_that("Test that all checks only produce one of PASS / FAIL / WARNING when run with EES robot tests", {
  test_dir <- tempdir()
  for (robot in ees_robot_test_data) {
    test_dir = tempdir()
    paths <- write_ees_files(robot$data, robot$meta, test_dir, robot$robot_name)
    results <- screen_csv(paths$data, paths$meta)$results$result |> unique()
    expect_true(
      all(results %in% c("PASS", "WARNING", "FAIL"))
    )
    file.remove(paths$data_path)
    file.remove(paths$meta_path)
  }
