test_that("passes when all filter groups in meta are present in data", {
  ...
  expect_equal(check_meta_fil_grp_match(data, meta)$result, "PASS")
  expect_no_error(check_meta_fil_grp_match(
    data,
    meta,
    stop_on_error = TRUE
  ))
...
test_that("fails when one filter group is missing from data", {
  data <- data.frame(sex = c("M", "F"))
  meta <- data.frame(filter_grouping_column = c("sex", "education_phase"))
  expect_equal(check_meta_fil_grp_match(data, meta)$result, "FAIL")
  expect_error(check_meta_fil_grp_match(data, meta, stop_on_error = TRUE))
})
