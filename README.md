...
You can use the `list_checks()` function to list out the current set of
checks ran by `screen_csv()`, e.g.:

``` r
# See the printed version in console
eesyscreener::list_checks()

# Export as a data frame
df <- eesyscreener::list_checks(print = FALSE)
```
