...
Checks that all ward_code and ward_name combinations in the data file
are valid. For Ward rows, all combinations must be valid. For non-Ward
rows, only non-empty, non-NA combinations are checked. Rows where
ward_code is "x" (the GSS not-available code) are excluded from non-Ward
checks.

## Usage

``` r
check_geog_ward_combos(data, verbose = FALSE, stop_on_error = FALSE)
```
...
If either ward column is absent from the data, the check passes
immediately.
