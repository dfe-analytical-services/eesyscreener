...
the data file are valid against the standard geographies lookup.
...
No geographic_level distinction is made (unlike most other geography
checks). All rows are filtered the same way before checking: rows where
old_la_code is NA are excluded, and rows where new_la_code is "x" (the
GSS not-available code) are excluded. All remaining combinations —
including those with blank or NA new_la_code — are checked and will fail
if not found in the lookup.
...
NA old_la_code would be checked without that filter and incorrectly
fail, since not all LAs have an old code.
