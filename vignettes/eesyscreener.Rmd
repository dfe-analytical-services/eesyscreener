...
This is a package designed to provide documentation and reusable code supprting the data standards for the explore education statistics (EES) platform. There may be other uses for the code in this package, however, supprting explore education statistics is it's primary purpose.
...
- Providing the logic behind the R Shiny app data screener, used to check files before upload
- Using that logic within EES itself through a plumber API, screening uploads into the service directly
...
- Returns one of the following outputs:
  ...
  - Console messages [Rich Q - should this be a verbosity option instead / still return structured list]
...
- TODO: We should do in a way that can be parrallelised, so users can speed up the screening for larger files if their machine allows (particularly helpful for the API and Shiny app)
